...
if($mode=="login"){
    ...
    if ( empty($row) ) {
        ...
        if($ck_auto=="1") {
            ...
            CookieUtil::setCookieP3pMd5('icm_adm_ck_userid', $row["userid"], 86400 * 31);
            CookieUtil::setCookieP3pMd5('icm_adm_ck_auto', "icm_adm_auto_login", 86400 * 31);
            //CookieUtil::setCookieP3pMd5('blm_ck_auto', $key, 86400 * 31);
            
        } else{
            CookieUtil::removeCookie("icm_adm_ck_userid");
            CookieUtil::removeCookie("icm_adm_ck_auto");
        }
        
        $row["passwd"] = "";

        $arrGrade = explode('|+|', $row['grade']);
        $arrGradeAlarm = explode('|+|', $row['grade_alarm']);

        for ($i=0;$i<count($arrGrade);$i++) {
            $row['grade_'.$i] = $arrGrade[$i];
        }

        for ($i=0;$i<count($arrGradeAlarm);$i++) {
            $row['grade_alarm_'.$i] = $arrGradeAlarm[$i];
        }
