...
<?php                                     	
	foreach($arrInsuranceCompanyUpload as $key => $value) {
?>
...
?>
                    <tr>
                        ...
                        <td style="text-align:center;">
							<a href="/admin/contract/contract_adm_list.php?_fg_link=Y&_grp_code=<?=$row["grp_code"]?>" target="_blank" class="button green xxsmail">보기</a>
						</td>
                        <td style="text-align:center;">
							<a href="#" name="btnDel" company_type="<?=$row["company_type"]?>" grp_code="<?=$row["grp_code"]?>" class="button red xxsmail">삭제</a>
						</td>
                    </tr>
<?php
    }
...
$(document).on("click", "a[name=btnDel]", function() {
    let grp_code = $(this).attr('grp_code');
    let company_type = $(this).attr('company_type');

    if (confirm("정말 해당 작업을 삭제하시겠습니까?    ")) {

        $.ajax({
            url: '/ajax/ajax_upload_data.php',
            type: 'POST',
            dataType: "json",
            async: true,
            cache: false,
